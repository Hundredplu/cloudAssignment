...
<html>
	...
	<div class="container">
        ...
        <?php
        global $mvtitle, $img, $des, $mvduration, $showing, $end;
        ...
        if (!empty($_POST)){           
            ...
            (isset($_POST['txtduration']))?
            $mvduration = trim($_POST['txtduration']):
                $mvduration = "";
            
            (isset($_POST['txtshowing']))?
            ...
            if (empty($error)){
                ...
                $sql = "INSERT INTO movies (title, cover_img, description, duration, date_showing, end_date) VALUES (?,?,?,?,?,?)";
                $statement = $connection ->prepare($sql);
                $statement -> bind_param('ssssss', $mvtitle, $img, $des, $mvduration, $showing, $end);
                $statement -> execute();
        ...
        if (isset($_FILES['fsimg'])){          
            ...
            if(!empty($update_image)){
                ...
                $image_update_query = mysqli_query($conn, "UPDATE movies SET cover_img = '$update_image' WHERE title = '$mvtitle'") or die('query failed');
        ...
        <div id="rcorners1" class="vl">              
                ...
                <h1 style="margin-left: 370px; font-size: 40px;">Add Page</h1>
                <table>
            ...
            <input style="padding: 9px 25px; background-color: rgba(0, 136, 169, 1); border: none; border-radius: 50px; cursor: pointer; transition: all 0.3s ease 0s; margin-top: 20px; font-size: 20px;" type="submit" value="Add" name="btnadd" />
